...
# tesseractのページセグメンテーション候補（7=単一行、8=単一単語）
OCR_PSM_CANDIDATES = (7, 8)
...
LOG_LEVEL = "INFO"
# ===================== 二値化設定 =====================
# 要調整-6: 二値化の固定しきい値（0-255）。
# 値を下げるほど「より黒い画素だけ」を黒と判定し、グレー(影・紙のムラ)を弾きやすくなる。
# 値を上げるほど、グレーも黒側に取り込まれやすくなる。
# 学籍番号の文字が真っ黒なカードなら 60〜100 あたりから調整開始を推奨。
OCR_BINARY_THRESHOLD = 85

# Trueの場合、しきい値を自動決定するOtsu法を使う。
# グレーまで黒判定されてしまう場合は False にして OCR_BINARY_THRESHOLD を手動調整する。
OCR_USE_OTSU = False